...
var is_ie11   = (navigator.userAgent.match(/Trident.*rv\:11\./) != null);
...
var ck_is_ie  = (userAgent.indexOf('msie') > 0 || userAgent.indexOf('trident') > 0);
...
{
	if (document.characterSet.toUpperCase() != 'UTF-8' && typeof text == 'string')
	{
		var tmptext = '', aux = '',
			codepoint, ucode, unicodematches;

		for (var i = 0, j = 1; i < text.length; i+=j)
		{
			aux = text.substring(i);
			codepoint = aux.codePointAt(0);

			if (codepoint > 0xFFFF)
			{
				j = 2;

				// Use this code for Alt codes
				ucode = '&#' + codepoint + ';';

				// This is to separate emojis from unicode special characters
				unicodematches = aux.substring(2).match(/(\u200D[^\uFE0F]*[\uFE0F])*(\u200D[\uD800-\uDBFF][\uDC00-\uDFFF])*/g);

				if (unicodematches && unicodematches[0].length > 0)
				{
					// This is a surrogate pair so more processing is needed
					j += unicodematches[0].length;

					for (var k = 0; k < unicodematches[0].length; k++)
					{
						var tmpcodepoint = unicodematches[0].codePointAt(k);

						// Skip low surrogates [\uDC00-\uDFFF] since they have no representation
						if (tmpcodepoint < 56320 || tmpcodepoint > 57343)
						{
							ucode += '&#' + tmpcodepoint + ';';
						}
					}
...
function set_cookie(name, value, expires)
{
	...
	document.cookie = name + '=' + escape(value) + '; path=/' + (typeof expires != 'undefined' ? '; expires=' + expires.toGMTString() : '') + '; domain=' + COOKIEDOMAIN;
}
...
function toggle_collapse(objid, forcestate)
{
	...
	var obj,
		...
		cel,
		collapseid = '';
	var collapse = jQuery('[id^=collapseobj_' + objid + ']');

	for (var i = 0; i < collapse.length; i++)
	{
		collapseid = collapse[i].id;
		obj = fetch_object(collapseid);
		img = fetch_object(collapseid.replace('collapseobj_','collapseimg_'));
		cel = fetch_object(collapseid.replace('collapseobj_','collapsecel_'));
		...
		if (obj.style.display == 'none' || 'open' == forcestate)
		{
			...
			if (MASTERSTYLEID == -2)
			{
				jQuery(event.target).addClass('m-toggled');
			}
			...
			if (img)
			{
				if (MASTERSTYLEID == -2)
				{
					if (jQuery(img).hasClass('fa-minus'))
					{
						jQuery(img).removeClass('fa-minus').addClass('fa-plus');
					}
					else
					{
						jQuery(img).removeClass('fa-plus').addClass('fa-minus');
					}
				}
				else
				{
		...
		else if (obj.style.display != 'none' || 'closed' == forcestate)
		{
			...
			if (MASTERSTYLEID == -2)
			{
				jQuery(event.target).removeClass('m-toggled');
			}
...
{
	var collapsed = fetch_cookie(COOKIE_PREFIX + 'vbulletin_collapse');
	var tmp = new Array();
	...
	set_cookie(COOKIE_PREFIX + 'vbulletin_collapse', tmp.join('\n'), expires);
}

// #############################################################################
// Collapsible with expand as default element handlers

/**
* Toggles the expand state of an object, and saves state to 'vbulletin_expand' cookie
*
* @param	string	Unique ID for the collapse group
*
* @return	boolean	false
*/
function toggle_expand(objid, forcestate)
{
	if (!is_regexp)
	{
		return false;
	...
	var obj,
		...
		cel,
		expandid = '';
	var expand = jQuery('[id^=expandobj_' + objid + ']');

	for (var i = 0; i < expand.length; i++)
	{
		expandid = expand[i].id;
		obj = fetch_object(expandid);
		img = fetch_object(expandid.replace('expandobj_','expandimg_'));
		cel = fetch_object(expandid.replace('expandobj_','expandcel_'));
		...
		if (obj.style.display == 'block' || 'closed' == forcestate)
		{
			obj.style.display = 'none';

			if (!forcestate)
			{
				save_expanded(objid, false);
			}
			...
			if (img)
			{
				...
				{
					img_re = new RegExp("_expanded\\.(gif|png)$");
					img.src = img.src.replace(img_re, '.$1');
				}
			}
			...
			if (cel)
			{
				cel_re = new RegExp("^(thead|tcat)(_expanded)$");
				cel.className = cel.className.replace(cel_re, '$1');
			}
		}
		else if (obj.style.display != '' || 'open' == forcestate)
		{
			obj.style.display = 'block';

			if (!forcestate)
			{
				save_expanded(objid, true);
			}
			...
			if (img)
			{
				...
				{
					img_re = new RegExp("\\.(gif|png)$");
					img.src = img.src.replace(img_re, '_expanded.$1');
				}
			}
			if (cel)
			{
				cel_re = new RegExp("^(thead|tcat)$");
				cel.className = cel.className.replace(cel_re, '$1_expanded');
			}
		}
	}
	return false;
...
* @param	boolean	Add a cookie
*/
function save_expanded(objid, addexpanded)
{
	var expanded = fetch_cookie(COOKIE_PREFIX + 'vbulletin_expand');
	var tmp = new Array();
	...
	if (expanded != null)
	{
		...
		{
			if (YAHOO.lang.hasOwnProperty(expanded, i) && expanded[i] != objid && expanded[i] != '')
			{
				tmp[tmp.length] = expanded[i];
			}
		}
	}
	...
	expires = new Date();
	expires.setTime(expires.getTime() + (1000 * 86400 * 365));
	set_cookie(COOKIE_PREFIX + 'vbulletin_expand', tmp.join('\n'), expires);
}
...
/**
* Submits the pagenav form on NextGen style
*/
vBpagenav.prototype.form_gotopage_ng = function(e)
{
	var _inputs = fetch_tags(this, 'input'),
		_pagenum = 0;
	for (var i = 0; i < _inputs.length; i++)
	{
		if (_inputs[i].name.indexOf('pagenav_itxt') != -1)
		{
			_pagenum = parseInt(_inputs[i].value, 10);
			break;
		}
	}
	...
	if (_pagenum > 0)
	{
		window.location = this.getAttribute('data-addr') + '&page=' + _pagenum;
	}
	return false;
};
...
function PostBit_Init(obj, postid)
{
	...
	if (typeof vB_Photoswipe != 'undefined')
	{
		if (typeof postid != 'undefined' && typeof postid != 'null')
		{
			var parentobj = $(obj).parents('[id^=posts]')[0];
			init_postbit_photoswipe(parentobj);
		}
		else
		{
			init_postbit_photoswipe(obj);
		}
	}
...
function vBulletin_init()
{
	...
	if (typeof vBmenu == 'object')
	{
		...
		if (MASTERSTYLEID == -4)
		{
			var pagenavs = fetch_tags(document, 'div');
		}
		else
		{
		...
		for (var n = 0; n < pagenavs.length; n++)
		{
			if (pagenavs[n].hasChildNodes() && pagenavs[n].firstChild.name && pagenavs[n].firstChild.name.indexOf('PageNav') != -1)
			{
				...
				pagenavs[n].title = '';
				//The following check prevents the 1/x on the magcbar pave navigation from being cleared
				if (pagenavs[n].firstChild.className.indexOf('magicpagenav') == -1)
				{
		...
		// process the NextGen pagenavs popup form
		var pagenavs_ng = fetch_tags(document, 'li'),
			pagenavform,
			pagenavrole;
		for (n = 0; n < pagenavs_ng.length; n++)
		{
			pagenavrole = pagenavs_ng[n].getAttribute('data-role');
			if (pagenavrole != null && typeof pagenavrole != 'undefined' && pagenavrole.indexOf('PageNav') != -1)
			{
				pagenavform = fetch_tags(pagenavs_ng[n], 'form')[0];
				pagenavform.gotopage = vBpagenav.prototype.form_gotopage_ng;
			}
		}
...
/**
* Function to get form elements including the
* elements added by the attribute form="formid"
*
* @param	string	Form element ID
*/
function fetch_form_elements(formid)
{
	...
	if (is_ie || is_ie11)
	{
		var formdata = jQuery('[form='+ formid + ']');
		for (var i = 0; i < formdata.length; i++)
		{
			elems.push(formdata[i]);
		}
	}
	elems = elems.concat(Array.prototype.slice.call(formobj.elements));

	return elems;
}

/*
	Wrapper code to run scripts
*/
var _pgWrapper = _pgWrapper || [];
_pgWrapper.cmd = _pgWrapper.cmd || [];
_pgWrapper.data = _pgWrapper.data || [];

function runAllScripts(_pgWrapper) {
	console.log('Running all scripts');
	for (var key in _pgWrapper.cmd) {
		runScript(_pgWrapper.cmd[key]);
	}
}

function runScript(item) {
	for (var innerkey in item) {
		console.log('Running: ' + innerkey);
		item[innerkey].call();
	}
}
...
* @param	string	Cookie name
*/
function clear_storage(prefix)
{
	if (window.localStorage)
	{
		prefix = prefix || COOKIE_PREFIX;
		var length =  prefix.length;
		var items = [], i;

		// If we have loaded the editor, clear the stored auto-save keys
		vBulletinStorage && vBulletinStorage.clear();

		// Iterate over localStorage and insert the keys that meet the condition into items
		for (i = 0; i < localStorage.length; i++)
		{
			if (localStorage.key(i).substring(0, length) == prefix)
			{
				items.push(localStorage.key(i));
			}
		}

		// Iterate over items and remove the items by key
...
/**
* Add GA tracking for search box forms and tag search forms
*
* @param	string Category name
*			string Action
*			object Cuttent Form Object
*			string Value to Indendify which part the GA is tracking
*			   	-1: Default
*				0: Threads
*				1: posts
*				2: Google Custom Search
*/
function searchform_ga_tracking(category, action, obj) {
	var keytag = jQuery(obj).find("input[id='tag_input']").val();
	if (typeof keytag !== 'undefined' && keytag !== '') {
		vB_Analytics.push(['event', {category: category, action: action, label: keytag}]);
	}
	else {
		var keyword = jQuery(obj).find("input[name='query']").val();
		var keytag = jQuery(obj).find("input[id='tag_add_input']").val();

		if (typeof keytag !== 'undefined' && keytag !== '') {
			vB_Analytics.push(['event', {category: 'searchpage', action: 'tagsearch', label: keytag}]);
		}

		if (action == 'searchnav') {
			if (jQuery(obj).find("input[name='do']").val() == 'showgcs') {
				action = 'searchgoogle';
			} else {
				action = (jQuery(obj).find("input[name='showposts']").val() == '1') ? 'searchposts' : 'searchthreads';
			}
		} else if (action == 'searchbox') {
			if (jQuery(obj).find("input[name='customsearch']").is(':checked')) {
				action = 'searchgoogle';
			} else {
				action = (jQuery(obj).find("input[name='showposts']:checked").val() == '1') ? 'searchposts' : 'searchthreads';
			}
		}

		if (keyword !== '') {
			vB_Analytics.push(['event', {category: category, action: action, label: keyword}]);
		}

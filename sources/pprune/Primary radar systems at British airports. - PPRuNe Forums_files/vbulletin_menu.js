...
function vB_Popup_Menu(controlkey, noimage, noslide)
{
	...
	this.parentmenuname = this.controlkey.split('.')[0];
...
vB_Popup_Menu.prototype.init_menu = function()
{
	...
	if (this.menuobj && !this.menuobj.initialized)
	{
		...
		if (MASTERSTYLEID == -3)
		{
				document.getElementById(this.parentmenuname).appendChild(document.getElementById(this.menuname));
		}
	}
};
...
{
	var tags = new Array("td", "li", "div");
	for (var j = 0; j < tags.length; j++)
	{
		var blocks = fetch_tags(this.menuobj, tags[j]);
		for (var i = 0; i < blocks.length; i++)
		{
			if (jQuery(blocks[i]).hasClass('vbmenu_option'))
			{
...
vB_Popup_Menu.prototype.show = function(obj, instant)
{
	...
	if (this.slide_open)
	{
		this.menuobj.style.clip = ck_is_ie ? 'auto' : 'rect(auto, 0px, 0px, auto)';
	}
	...
	{
		this.menuobj.style.clip = ck_is_ie ? 'auto' : 'rect(auto, auto, auto, auto)';
	}
...
{
	this.offset_left = jQuery(obj).offset().left;
	this.pos = this.fetch_offset(obj);
	this.leftpx = this.pos['left'];
	this.toppx = this.pos['top'] + obj.offsetHeight;

	if ((this.offset_left + this.menuobj.offsetWidth) >= document.body.clientWidth && (this.offset_left + obj.offsetWidth - this.menuobj.offsetWidth) > 0)
	{
...
vB_Popup_Menu.prototype.slide = function(clipX, clipY, opacity)
{
	if (this.direction == 'left' && (clipX < this.menuobj.offsetWidth || clipY < this.menuobj.offsetHeight))
	{
		...
		this.menuobj.style.clip = ck_is_ie ? 'auto' : 'rect(auto, ' + clipX + 'px, ' + clipY + 'px, auto)';
	...
	else if (this.direction == 'right' && (clipX > 0 || clipY < this.menuobj.offsetHeight))
	{
		...
		this.menuobj.style.clip = ck_is_ie ? 'auto' : 'rect(auto, ' + this.menuobj.offsetWidth + 'px, ' + clipY + 'px, ' + clipX + 'px)';
		this.slidetimer = setTimeout("vBmenu.menus[vBmenu.activemenu].slide(" + clipX + ", " + clipY + ", " + opacity + ");", 0);
	}
	else
	{
...
{
	// Now that we have positioned elements in the page we need to check parents
	var position = jQuery(obj).offsetParent().css('position');

	if (position == 'fixed' || position == 'relative' || position == 'absolute')
	{
		// Find the position of obj relative to the positioned parent
		var rect = jQuery(obj),
			menu = rect.offset(),
			pmenu = rect.offsetParent().offset();

		return { 'left': menu.left - pmenu.left, 'top': menu.top - pmenu.top };
	}
...
{
	jQuery(this).removeClass('vbmenu_option vbmenu_option_alink').addClass('vbmenu_hilite');
	if (this.islink)
	{
		jQuery(this).addClass('vbmenu_hilite_alink');
	}
...
{
	jQuery(this).removeClass('vbmenu_hilite vbmenu_hilite_alink').addClass('vbmenu_option');
	if (this.islink)
	{
		jQuery(this).addClass('vbmenu_option_alink');
	}
